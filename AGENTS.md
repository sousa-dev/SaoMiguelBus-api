...
Exception, so a quiet island still shows a number: if a record is `unavailable` or a recorded `0` **and** it is 06:00–18:59 in Atlantic/Azores, the endpoint makes exactly one real vendor call per operator through the existing cached fleet path. A **completely missing** record (first deploy, a Redis flush — nothing cached for this operator at all) refreshes regardless of time of day: AzoresBus is kept warm around the clock by its own background route-index sweep, but MiniBus has no equivalent, so waiting for daytime could mean an all-night blank hub after a deploy for no reason. Either way it is gated by a 5-minute "already tried" marker (set regardless of outcome) so concurrent hub requests can't each trigger their own, and outside that exception a stale/empty record is simply served as-is. Vendor failures during this top-up are recorded as `unavailable` — this endpoint never returns a 5xx over a tracking problem.
...
- `POST /api/v3/azoresbus/live-activities` — `{pushToken, environment, activityKey, legs:[{tripId,startsAt,endsAt}], expiresAt}` → 201. Re-registering the same `pushToken` updates the row (a relaunch, a language change) rather than duplicating it; a previously-ended registration is revived. Max 5 legs. Gated by `trackingEnabled`, same 503 as every other azoresbus endpoint.
- `DELETE /api/v3/azoresbus/live-activities/<push_token>` → 204. Marks `ended_at` rather than deleting — the beat task still owes the activity a final `event: "end"` push so the card dismisses itself instead of sitting frozen until Apple's own ceiling expires it. Idempotent: an unknown or already-ended token is not an error.
- Model: `LiveActivityRegistration`. Stores no personal data — a push token, trip ids and times, nothing that identifies a person.
- Beat task `azoresbus.push_live_activities` (every minute, `azoresbus/tasks.py`, registered by `migrations/0004_periodic_task_live_activities.py`): for each unexpired registration, picks the current leg by clock (`services_live_activity_push.current_leg`, mirroring `currentLegIndex` in the app and `LiveTripPoller.kt` in the Android module), calls the same `live_for_trips` the `/trips/live` endpoint uses, and pushes a content-state built by `snapshot_from_live_row` — one implementation of "what does a live row mean" per language (TypeScript, Kotlin, Python), kept in step by hand. Sends `event: "end"` once the last leg's `endsAt` plus grace has passed. Lock-guarded (`PUSH_LOCK_KEY`/`TTL`) so a slow run cannot overlap the next tick.
- `azoresbus/apns.py` — ES256 provider-token auth (`APNS_KEY_ID`/`APNS_TEAM_ID`/`APNS_KEY_P8`/`APNS_BUNDLE_ID` env, see `.env.example`), cached ~45 min (Apple accepts a token for about an hour and rate-limits minting one to roughly once per 20 min). `httpx[http2]` — the one HTTP/2 dependency in this codebase, since raw APNs requires it and `requests` cannot speak it. A `410`/`400 BadDeviceToken` response is terminal and ends the registration; anything else just increments `failure_count` and is retried next tick.
- ActivityKit rejects a push over 4KB — `live_activity_payload` builds `{"aps": {"timestamp", "event", "content-state"}}` and nothing larger.
