"""DRF throttle for marketplace write endpoints."""
...
class MarketplaceWriteThrottle(SimpleRateThrottle):
    """Throttle writes per (island, session); reads are never throttled."""

    scope = 'marketplace_write'

    def get_cache_key(self, request, view):
        if request.method in SAFE_METHODS:
            return None  # SimpleRateThrottle treats None as "no throttle"
