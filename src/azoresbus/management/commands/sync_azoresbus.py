...
class Command(BaseCommand):
    ...
    def _execute(self, island, dates, near, far, holidays,
        ...
        try:
            report = run_sync(
                island,
                dates=dates if self._explicit else None,
                full=bool(far),
                no_prune=no_prune,
        ...
        self.stdout.write(self.style.SUCCESS(
            ...
            f'{report["trips"]} trips in {report["requests"]} requests'
        ))
        self.stdout.write(
            f'retirement        '
            f'{"applied" if report["retirement"]["allowed"] else "skipped"} '
            f'-- {report["retirement"]["reason"]}'
        )
