...
def vehicles_list_view(request: Request) -> Response:
    err = _require_island(request)
    if err:
        return err
    ...
    with for_island(request.island):
        try:
            vehicles, meta = get_fleet_tracking(request.island)
        except MinibusTrackingError as exc:
            return Response(
                {'error': {'code': 'tracking_unavailable', 'message': str(exc)}},
    ...
    response = Response(
        {
            'vehicles': vehicles,
            **build_tracking_response_meta(meta),
            **build_meta_payload(request.island),
        },
    )
    response['X-Minibus-Tracking-Cache'] = meta.cache_status
    return response
...
@permission_classes([AllowAny])
@throttle_classes([MinibusTrackingThrottle])
def vehicle_detail_view(request: Request, tracking_id: str) -> Response:
    err = _require_island(request)
    if err:
        return err
    ...
    with for_island(request.island):
        try:
            vehicle, meta = get_vehicle_tracking(request.island, tracking_id)
        except MinibusTrackingNotFoundError as exc:
            return Response(
                {'error': {'code': 'tracking_not_found', 'message': str(exc)}},
                status=status.HTTP_404_NOT_FOUND,
            )
        except MinibusTrackingError as exc:
            return Response(
